...
import { X, Phone, Mail, CheckCircle2, ShieldCheck } from "lucide-react";
    ...
    >
      ...
      <div className="relative w-full max-w-4xl bg-[#FAF8F4] border border-[#3A2620]/20 rounded-2xl overflow-hidden shadow-2xl z-10 grid grid-cols-1 md:grid-cols-2 max-h-[90vh]">
        ...
        <div className="p-6 sm:p-8 flex flex-col justify-between overflow-y-auto max-h-[60vh] md:max-h-full">
          ...
          {/* Ordering / Inquiry CTA */}
          <div className="pt-6 mt-6 border-t border-[#3A2620]/10 space-y-3">
            <div className="text-[11px] text-[#3A2620]/70 font-medium">
              For direct orders or fitting inquiries:
            </div>
            <div className="grid grid-cols-2 gap-3">
              <a
                href={`tel:${siteConfig.phoneRaw}`}
                className="inline-flex items-center justify-center gap-2 py-2.5 px-3 bg-[#3A2620] text-[#FAF8F4] text-xs font-medium rounded-lg hover:bg-[#1C1B1B] transition-colors"
              >
                <Phone className="w-3.5 h-3.5" /> Call {siteConfig.phone}
              </a>
              <a
                href={`mailto:${siteConfig.email}?subject=Inquiry: ${product.name}`}
                className="inline-flex items-center justify-center gap-2 py-2.5 px-3 border border-[#3A2620]/30 text-[#3A2620] text-xs font-medium rounded-lg hover:bg-[#F5F0E9] transition-colors"
              >
